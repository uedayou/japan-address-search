...
module.exports = function(address) {
  ...
  // 住所部分一致検索
  const ukeys = Object.keys(upper).filter(k=>k.match(new RegExp(normalized)));
  for (const key of ukeys) {
    for (const obj of upper[key]) {
      res.push(fix(obj, ""));
    }
  }
  const lkeys = Object.keys(lower).filter(k=>k.match(new RegExp(normalized)));
  for (const key of lkeys) {
    for (const obj of lower[key]) {
      res.push(fix(obj, ""));
    }
  }
  res = res.filter((v1,i1,a1) =>
    a1.findIndex(v2 => v1.code===v2.code) === i1);
  return res;
};
